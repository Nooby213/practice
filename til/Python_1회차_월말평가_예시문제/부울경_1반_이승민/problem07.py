...
def tidy_up_company(email_list): # 이메일 리스트 매개변수
    mail_dict = {i : 0 for i in email_list} # 이메일 종류별로 딕셔너리 초기값
    for mail in email_list: # 이메일 리스트 순회
        mail_dict[mail] += +1 # 이메일 종류 별로 숫자 +1
    return mail_dict
